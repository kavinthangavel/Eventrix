...
export const createEvent = async ({
  ...
  try {
    console.log(`Attempting to find organizer with clerkId: ${userId}`);
    ...
    if (!organizer) {
      ...
      throw new Error(`Organizer not found for clerkId: ${userId}. Total users in database: ${userCount}`);
    }
    ...
    const newEvent = await Event.create({
      ...
      organizer: organizer._id, // Assign organizer's ObjectId
    });
...
export const getEventsByUser = async ({
  ...
  try {
    ...
    // Find the user by clerkId to get their _id
    console.log(`Searching for user with clerkId: ${userId}`);
    const user = await User.findOne({ clerkId: userId });
    if (!user) {
      // Check if any users exist in the database
      const userCount = await User.countDocuments({});
      throw new Error(`User not found for clerkId: ${userId}. Total users in database: ${userCount}`);
    }
    console.log(`Found user: ${user._id}`);

    const skipAmount = (page - 1) * 6;
    const query = { organizer: user._id };
    console.log(`Querying events for organizer: ${user._id}`);
    ...
    console.log(`Found ${eventsCount} events for organizer ${user._id}`);
